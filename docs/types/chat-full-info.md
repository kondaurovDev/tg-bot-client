...
| rating | [`UserRating`](user-rating.md) | No | For private chats, the rating of the user if any |
| first_profile_audio | [`Audio`](audio.md) | No | For private chats, the first audio added to the profile of the user |
| unique_gift_colors | [`UniqueGiftColors`](unique-gift-colors.md) | No | The color scheme based on a unique gift that must be used for the chat&#39;s name, message replies and link previews |
