...
| from | [`User`](/api/types/user/) | Yes | The user that chose the result |
| query | `string` | Yes | The query that was used to obtain the result |
| location | [`Location`](/api/types/location/) | No | Sender location, only for bots that require user location |
