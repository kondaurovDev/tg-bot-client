...
| web_app | [`WebAppInfo`](/api/types/web-app-info/) | No | Description of the Web App that will be launched when the user presses the button The Web App will be able to send an arbitrary message on behalf of the user using the method answerWebAppQuery Available only in private chats between a user and the bot Not supported for messages sent on behalf of a Telegram Business account. |
| login_url | [`LoginUrl`](/api/types/login-url/) | No | An HTTPS URL used to automatically authorize the user Can be used as a replacement for the Telegram Login Widget. |
...
| switch_inline_query_chosen_chat | [`SwitchInlineQueryChosenChat`](/api/types/switch-inline-query-chosen-chat/) | No | If set, pressing the button will prompt the user to select one of their chats of the specified type, open that chat and insert the bot&#39;s username and the specified inline query in the input field Not supported for messages sent in channel direct messages chats and on behalf of a Telegram Business account. |
| copy_text | [`CopyTextButton`](/api/types/copy-text-button/) | No | Description of the button that copies the specified text to the clipboard. |
| callback_game | [`CallbackGame`](/api/types/callback-game/) | No | Description of the game that will be launched when the user presses the button . NOTE: This type of button must always be the first button in the first row. |
