...
|-------|------|----------|-------------|
| chat | [`Chat`](/api/types/chat/) | Yes | Chat to which the request was sent |
| from | [`User`](/api/types/user/) | Yes | User that sent the join request |
...
| invite_link | [`ChatInviteLink`](/api/types/chat-invite-link/) | No | Chat invite link that was used by the user to send the join request |
