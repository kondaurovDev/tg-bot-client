...
| accepted_gift_types | [`AcceptedGiftTypes`](/api/types/accepted-gift-types/) | Yes | Information about types of gifts that are accepted by the chat or by the corresponding user for private chats |
...
| photo | [`ChatPhoto`](/api/types/chat-photo/) | No | Chat photo |
| active_usernames | `string[]` | No | If non-empty, the list of all active chat usernames; for private chats, supergroups and channels |
| birthdate | [`Birthdate`](/api/types/birthdate/) | No | For private chats, the date of birth of the user |
| business_intro | [`BusinessIntro`](/api/types/business-intro/) | No | For private chats with business accounts, the intro of the business |
| business_location | [`BusinessLocation`](/api/types/business-location/) | No | For private chats with business accounts, the location of the business |
| business_opening_hours | [`BusinessOpeningHours`](/api/types/business-opening-hours/) | No | For private chats with business accounts, the opening hours of the business |
| personal_chat | [`Chat`](/api/types/chat/) | No | For private chats, the personal channel of the user |
| parent_chat | [`Chat`](/api/types/chat/) | No | Information about the corresponding channel chat; for direct messages chats only |
| available_reactions | [`ReactionType`](/api/types/reaction-type/)[] | No | List of available reactions allowed in the chat If omitted, then all emoji reactions are allowed. |
...
| pinned_message | [`Message`](/api/types/message/) | No | The most recent pinned message (by sending date) |
| permissions | [`ChatPermissions`](/api/types/chat-permissions/) | No | Default chat member permissions, for groups and supergroups |
...
| location | [`ChatLocation`](/api/types/chat-location/) | No | For supergroups, the location to which the supergroup is connected |
| rating | [`UserRating`](/api/types/user-rating/) | No | For private chats, the rating of the user if any |
| first_profile_audio | [`Audio`](/api/types/audio/) | No | For private chats, the first audio added to the profile of the user |
| unique_gift_colors | [`UniqueGiftColors`](/api/types/unique-gift-colors/) | No | The color scheme based on a unique gift that must be used for the chat&#39;s name, message replies and link previews |
| paid_message_star_count | `number` | No | The number of Telegram Stars a general user have to pay to send a message to the chat |
