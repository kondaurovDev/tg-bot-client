...
| caption_entities | [`MessageEntity`](/api/types/message-entity/)[] | No | List of special entities that appear in the caption, which can be specified instead of parse_mode |
...
| input_message_content | [`InputMessageContent`](/api/types/input-message-content/) | No | Content of the message to be sent instead of the video This field is required if InlineQueryResultVideo is used to send an HTML-page as a result (e.g., a YouTube video). |
