...
| thumbnail | [`PhotoSize`](/api/types/photo-size/) | No | Sticker thumbnail in the .WEBP or .JPG format |
| emoji | `string` | No | Emoji associated with the sticker |
| set_name | `string` | No | Name of the sticker set to which the sticker belongs |
| premium_animation | [`File`](/api/types/file/) | No | For premium regular stickers, premium animation for the sticker |
| mask_position | [`MaskPosition`](/api/types/mask-position/) | No | For mask stickers, the position where the mask should be placed |
