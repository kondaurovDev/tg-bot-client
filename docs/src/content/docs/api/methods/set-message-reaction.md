...
| reaction | [`ReactionType`](/api/types/reaction-type/)[] | No | A JSON-serialized list of reaction types to set on the message Currently, as non-premium users, bots can set up to one reaction per message A custom emoji reaction can be used if it is either already present on the message or explicitly allowed by chat administrators Paid reactions can&#39;t be used by bots. |
