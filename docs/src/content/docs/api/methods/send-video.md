...
| video | [`InputFile`](/api/types/input-file/) \| `string` | Yes | Video to send Pass a file_id as String to send a video that exists on the Telegram servers (recommended), pass an HTTP URL as a String for Telegram to get a video from the Internet, or upload a new video using multipart/form-data More information on Sending Files » |
