...
| photo | [`InputFile`](/api/types/input-file/) \| `string` | Yes | Photo to send Pass a file_id as String to send a photo that exists on the Telegram servers (recommended), pass an HTTP URL as a String for Telegram to get a photo from the Internet, or upload a new photo using multipart/form-data The photo must be at most 10 MB in size The photo&#39;s width and height must not exceed 10000 in total Width and height ratio must be at most 20 More information on Sending Files » |
